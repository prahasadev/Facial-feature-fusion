...
def lab_color_transfer(source, target):
    ...
    for i in range(3):
        s_mean = np.mean(s_lab[:, :, i])
        s_std = np.std(s_lab[:, :, i])
        t_mean = np.mean(t_lab[:, :, i])
        t_std = np.std(t_lab[:, :, i])
...
def blend_feature(canvas_img, src_img, feature_name):
    ...
    dx1 = max(0, dx - dst_pad)
    dy1 = max(0, dy - dst_pad)
    dx2 = min(w_dst, dx + dw + dst_pad)
    dy2 = min(h_dst, dy + dh + dst_pad)
    ...
    sx1 = max(0, sx - src_pad)
    sy1 = max(0, sy - src_pad)
    sx2 = min(w_src, sx + sw + src_pad)
    sy2 = min(h_src, sy + sh + src_pad)
    
    src_crop = src_img[sy1:sy2, sx1:sx2].copy()
    local_src_pts = src_pts - np.array([sx1, sy1], dtype=np.int32)
    
    matrix, _ = cv2.estimateAffinePartial2D(local_src_pts, local_dst_pts, method=cv2.RANSAC)
    ...
    cw = dx2 - dx1
    ch = dy2 - dy1
    warped_src_crop = cv2.warpAffine(src_crop, matrix, (cw, ch), flags=cv2.INTER_LINEAR, borderMode=cv2.BORDER_REFLECT)
    cv2.imwrite(f"v9_{feature_name}_aligned.jpg", warped_src_crop)
    ...
    if feature_name == "mouth":
        ...
        mask_crop = cv2.GaussianBlur(mask_crop, (17, 17), 0)
    else:
        mask_crop = cv2.GaussianBlur(mask_crop, (11, 11), 0)
        
    matched_src_crop = lab_color_transfer(warped_src_crop, dst_crop)
    cv2.imwrite(f"v9_{feature_name}_color_matched.jpg", matched_src_crop)
...
def build_v9_splicer(photo_a_path, photo_b_path, photo_c_path, output_path):
    ...
    print("\n--- V9: Blending Nose ---")
    canvas = blend_feature(img_a, img_b, "nose")
    ...
    if not success:
        raise IOError("Could not save V9 output image.")
        
    print(f"\nV9 complete! Saved as {output_path}")
